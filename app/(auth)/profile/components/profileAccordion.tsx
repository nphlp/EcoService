...
const formatDate = (date: Date) => {
    ...
    const localDate = newDate.toLocaleDateString("fr-FR", {
        day: "2-digit",
        month: "2-digit",
        year: "numeric",
    });
...
export default function ProfileAccordion(props: ProfileAccordionProps) {
    ...
    const creationTime = formatTime(session.session.createdAt);
    const updateTime = formatTime(session.session.updatedAt);
    const expirationTime = formatTime(session.session.expiresAt);
